...
const ADMIN_EMAILS = ['rdgosmartins@gmail.com', 'massaro.joaog@gmail.com'];
const ADMIN_EMAIL  = ADMIN_EMAILS[0]; // compat — alguns trechos legados ainda referenciam o singular
...
// Espelho somente-leitura dos dados do bot de matchmaking Discord (YOKO Matchmaker).
// O SQLite do bot é a fonte da verdade; aqui só guardamos o resultado final de
// cada partida confirmada, pra alimentar a aba "Matchmaker" do dashboard.

const MM_PLAYER_INDEX_KEY = 'mm_player_index';
const MM_MATCH_INDEX_KEY  = 'mm_match_index';
const MM_MAX_MATCH_HISTORY = 500;

async function mmGetPlayer(env, discordId) {
    const raw = await env.AUTH_KV.get(`mm_player:${discordId}`);
    return raw ? JSON.parse(raw) : null;
}
async function mmPutPlayer(env, player) {
    await env.AUTH_KV.put(`mm_player:${player.discordId}`, JSON.stringify(player));
}

async function mmPrependIndex(env, key, id, max) {
    const raw = await env.AUTH_KV.get(key);
...
async function handleMe(request, env, cors) {
    ...
    return json({ id, email, displayName, avatarUrl, role, status, bandaiName, teamId }, 200, cors);
}
...
async function handleTeamsPost(request, env, cors) {
    ...
    const team = {
        ...
        color: body.color === undefined || body.color === null || body.color === '' ? null : String(body.color).trim().slice(0, 32),
        icon: body.icon === undefined || body.icon === null || body.icon === '' ? null : String(body.icon).trim().slice(0, 32),
        createdAt: now,
        updatedAt: now,
    };
...
async function handleTeamPut(request, env, cors, id) {
    ...
    if (body.color !== undefined) {
        team.color = body.color === null || body.color === '' ? null : String(body.color).trim().slice(0, 32);
    }
    if (body.icon !== undefined) {
        team.icon = body.icon === null || body.icon === '' ? null : String(body.icon).trim().slice(0, 32);
    }
    ...
    await appendAuditLog(env, {
        ...
        targetId: team.id,
        targetName: team.name,
        detail: 'name/color/icon updated',
    });
...
async function handleTeamDelete(request, env, cors, id) {
    ...
    const ids = raw ? JSON.parse(raw) : [];
    const assigned = (await Promise.all(ids.map(uid => getUser(env, uid)))).filter(u => u && u.teamId === id);
    if (assigned.length) {
        return json({
...
async function handlePromoteByEmail(request, env, cors) {
    const actor = await authenticate(request, env);
    if (!actor || actor.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    ...
    if (!user) return json({ error: `Usuário com email ${email} não encontrado. Ele precisa se cadastrar primeiro.` }, 404, cors);
    ...
    await putUser(env, user);
    await removeIndex(env, 'pending_index', user.id);
    await appendAuditLog(env, { actorId: actor.id, actorName: actor.displayName, action: 'promote_by_email', targetId: user.id, targetName: user.displayName, detail: email });
    return json({ ok: true, id: user.id, email: user.email, role: user.role, status: user.status }, 200, cors);
}
...
// Bulk read: devolve o cache de eventos de TODOS os bandaiIds já sincronizados,
// de uma vez só. Usado pelo Global Rankings / Sync All para que qualquer pessoa
// que abra o app veja os dados sincronizados por qualquer outro membro do time —
// e não só os dados que já estão no localStorage do próprio navegador.
async function handleCacheAllGet(request, env, cors) {
    const user = await authenticate(request, env);
    if (!user) return json({ error: 'Unauthorized' }, 401, cors);
    const result = {};
    let cursor;
    do {
        const page = await env.AUTH_KV.list({ prefix: 'cache:', cursor });
        for (const key of page.keys) {
            const raw = await env.AUTH_KV.get(key.name);
            if (!raw) continue;
            try {
                result[key.name.slice('cache:'.length)] = JSON.parse(raw);
...
async function handleAdminEditUser(request, env, cors, id) {
    ...
    let nextTeamId = previousTeamId;

    if (body.teamId !== undefined) {
        nextTeamId = body.teamId === null || body.teamId === '' ? null : String(body.teamId).trim();
        if (nextTeamId) {
    ...
    if (body.displayName !== undefined) {
        const nextDisplayName = String(body.displayName).slice(0, 64);
    ...
    if (body.teamId !== undefined && nextTeamId !== previousTeamId) {
    ...
    const action = body.teamId !== undefined && nextTeamId !== previousTeamId
        ? (nextTeamId ? 'assign_team' : 'clear_team')
        : 'edit_user';
    await appendAuditLog(env, {
        actorId: actor.id,
        actorName: actor.displayName,
        ...
        detail: changes.length ? `${changes.join(', ')} updated` : 'no changes',
    });
    return json({ ok: true }, 200, cors);
}
...
// ── OPTCG Agent Chat Proxy ────────────────────────────────────────────────────
// Recebe { messages: [...] } do frontend e repassa para a Anthropic API.
// A API key fica segura como variável de ambiente ANTHROPIC_API_KEY no Worker.

const AGENT_SYSTEM = `Você é um especialista no ONE PIECE CARD GAME (TCG) com conhecimento profundo das regras oficiais (Comprehensive Rules v1.2.0) e motor estratégico derivado de simulações de partidas.

NOTAÇÃO: "cX" = custo X (ex: c10 = custo 10). Código de deck: QTYxSET-NUM (ex: 4xOP16-091).

REGRAS OFICIAIS v1.2.0:
- Derrota: Leader leva dano com 0 Life OU deck esgota
- Deck: 1 Leader + 50 cartas (cores do Leader, máx 4x por ID) + 10 DON!!
- TURNO: Refresh → Draw → DON!!(+2; 1° jogador +1 no T1) → Main → End
- DON!!: dar ativo a Leader/Character = +1000pw NO SEU TURNO; retorna no Refresh
- BATALHA: Attack(restar atacante, alvo=Leader ou Character DESCANSADO) → Block(1 Blocker) → Counter → Damage(≥=win: Leader→dano/Character→KO; <: nada)
- [Rush]=ataca no turno que entra; [Double Attack]=2 danos; [Banish]=trasha Life sem Trigger
- [Blocker]=intercepta Leader; [Trigger]=ao sair do Life; [Unblockable]=sem blocker
- Texto da carta prevalece sobre as regras gerais

ESTRATÉGIA (simulações Shanks Red vs Luffy Impel Down):
MULLIGAN: manter SE c1-c2 jogável T1-T2 OU tutor OU Blocker. Fazer SE todas ≥c5 sem tutor.
CURVA: indo 1º → DON!! ímpar (T1=1,T2=3,T3=5...); indo 2º → par (T1=2,T2=4,T3=6...).
COUNTER: Life≥4 aceitar (cada Life = +1 carta na mão). Life=2 counterar. Life=1 usar tudo.
BLOCKER: só protege Leader. Não usar com Life≥4. Usar o de menor power suficiente.
BURN vs REMOVAL: Leader > KO Blocker > KO char caro. Removal obrigatório com 2+ Blockers ativos.
DON!!: nunca desperdiçar. Prioridade: jogar cartas > dar DON!! para KO > guardar Counter.
SHANKS LEADER (OP09-001): −1000pw ao atacante anula ataques de 5000pw — precisa DON!! no Leader.
LUFFY IMPEL DOWN (OP16-022): combo = 5 chars Impel Down + Let's Go!! + Boa Hancock Unblockable.

Quando DADOS DA BASE ou DADOS DO DECK forem fornecidos, use-os para responder com precisão.
Responda no mesmo idioma do usuário. Cite IDs de cartas quando relevante.`;

async function handleAgentChat(request, env, cors) {
    if (!env.ANTHROPIC_API_KEY) {
        return json({ error: 'ANTHROPIC_API_KEY não configurada no Worker.' }, 500, cors);
    }
    let body;
    ...
    if (!Array.isArray(messages) || !messages.length) {
        return json({ error: 'messages obrigatório' }, 400, cors);
    }

    // Rate limit simples: max 20 mensagens por conversa enviadas ao Worker
    if (messages.length > 40) {
        return json({ error: 'Conversa muito longa. Recarregue a página para iniciar nova sessão.' }, 429, cors);
    }
    ...
    if (!r.ok) {
        ...
        return json({ error: `Anthropic API: ${r.status}`, detail: err }, 502, cors);
    }
...
function newStoreId() { return 'str_' + crypto.randomUUID().replace(/-/g, '').slice(0, 12); }

async function getStore(env, id) {
    const raw = await env.AUTH_KV.get(`store:${id}`);
    return raw ? JSON.parse(raw) : null;
}
async function putStore(env, s) { await env.AUTH_KV.put(`store:${s.id}`, JSON.stringify(s)); }

async function handleStoresGet(request, env, cors) {
    const user = await authenticate(request, env);
    if (!user) return json({ error: 'Unauthorized' }, 401, cors);
    const raw  = await env.AUTH_KV.get('store_index');
    const ids  = raw ? JSON.parse(raw) : [];
    const list = (await Promise.all(ids.map(id => getStore(env, id)))).filter(Boolean);
    list.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    return json(list, 200, cors);
}

async function handleStoresPost(request, env, cors) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    let body;
    try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, cors); }
    ...
    const s = {
        ...
        address:   address ? String(address).slice(0, 256) : null,
        createdAt: new Date().toISOString(),
        createdBy: user.id,
    };

    await putStore(env, s);
    await appendIndex(env, 'store_index', s.id);
    await appendAuditLog(env, { actorId: user.id, actorName: user.displayName, action: 'create_store', targetId: s.id, targetName: s.name });
    return json(s, 201, cors);
}

async function handleStorePut(request, env, cors, id) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    const s = await getStore(env, id);
    if (!s) return json({ error: 'Not found' }, 404, cors);

    let body;
    try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, cors); }

    if (body.name    !== undefined) s.name    = String(body.name).slice(0, 128);
    if (body.city    !== undefined) s.city    = body.city    ? String(body.city).slice(0, 64)    : null;
    if (body.address !== undefined) s.address = body.address ? String(body.address).slice(0, 256) : null;

    await putStore(env, s);
    return json(s, 200, cors);
}

async function handleStoreDelete(request, env, cors, id) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    const s = await getStore(env, id);
    if (!s) return json({ error: 'Not found' }, 404, cors);

    await env.AUTH_KV.delete(`store:${id}`);
    await removeIndex(env, 'store_index', id);
    await appendAuditLog(env, { actorId: user.id, actorName: user.displayName, action: 'delete_store', targetId: id, targetName: s.name });
    return json({ ok: true }, 200, cors);
}

// ── Deck Mapping: Events (torneio + deck list) ─────────────────────────────────

function newDeckMapId() { return 'dmp_' + crypto.randomUUID().replace(/-/g, '').slice(0, 12); }

async function getDeckMap(env, id) {
    const raw = await env.AUTH_KV.get(`deckmap:${id}`);
    return raw ? JSON.parse(raw) : null;
}
async function putDeckMap(env, d) { await env.AUTH_KV.put(`deckmap:${d.id}`, JSON.stringify(d)); }

async function handleDeckMapsGet(request, env, cors) {
    const user = await authenticate(request, env);
    if (!user) return json({ error: 'Unauthorized' }, 401, cors);
    const raw  = await env.AUTH_KV.get('deckmap_index');
    const ids  = raw ? JSON.parse(raw) : [];
    const list = (await Promise.all(ids.map(id => getDeckMap(env, id)))).filter(Boolean);
    list.sort((a, b) => (b.date || '').localeCompare(a.date || ''));
    return json(list, 200, cors);
}

async function handleDeckMapsPost(request, env, cors) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    let body;
    try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, cors); }

    const { storeId, date, format, playerCount, entries } = body;
    if (!storeId || !date || !format) return json({ error: 'storeId, date e format são obrigatórios' }, 400, cors);

    const d = {
        id:          newDeckMapId(),
        storeId:     String(storeId).slice(0, 64),
        date:        String(date).slice(0, 10),
        format:      String(format).slice(0, 64),
        playerCount: Number(playerCount) || 0,
        entries: Array.isArray(entries)
            ? entries.slice(0, 512).map(e => ({
                playerId:   String(e.playerId   || '').slice(0, 64),
                playerName: String(e.playerName || '').slice(0, 64),
                isGuest:    !!e.isGuest,
                leaderId:   String(e.leaderId   || '').slice(0, 16),
              }))
            : [],
        createdAt: new Date().toISOString(),
        createdBy: user.id,
    ...
    await putDeckMap(env, d);
    await appendIndex(env, 'deckmap_index', d.id);
    await appendAuditLog(env, { actorId: user.id, actorName: user.displayName, action: 'create_deckmap', targetId: d.id, targetName: `${d.format} @ ${d.storeId}` });
    return json(d, 201, cors);
}

async function handleDeckMapPut(request, env, cors, id) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    const d = await getDeckMap(env, id);
    if (!d) return json({ error: 'Not found' }, 404, cors);

    let body;
    try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, cors); }

    if (body.storeId     !== undefined) d.storeId     = String(body.storeId).slice(0, 64);
    if (body.date        !== undefined) d.date        = String(body.date).slice(0, 10);
    if (body.format      !== undefined) d.format      = String(body.format).slice(0, 64);
    if (body.playerCount !== undefined) d.playerCount = Number(body.playerCount) || 0;
    if (body.entries     !== undefined) d.entries = Array.isArray(body.entries)
        ? body.entries.slice(0, 512).map(e => ({
            playerId:   String(e.playerId   || '').slice(0, 64),
            playerName: String(e.playerName || '').slice(0, 64),
            isGuest:    !!e.isGuest,
            leaderId:   String(e.leaderId   || '').slice(0, 16),
...
async function handleDeckMapDelete(request, env, cors, id) {
    const user = await authenticate(request, env);
    if (!user || user.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
    const d = await getDeckMap(env, id);
    if (!d) return json({ error: 'Not found' }, 404, cors);

    await env.AUTH_KV.delete(`deckmap:${id}`);
    await removeIndex(env, 'deckmap_index', id);
    await appendAuditLog(env, { actorId: user.id, actorName: user.displayName, action: 'delete_deckmap', targetId: id, targetName: `${d.format} @ ${d.storeId}` });
    return json({ ok: true }, 200, cors);
}

// ── Matchmaker handlers ────────────────────────────────────────────────────────

// POST /matchmaker/sync — chamado pelo bot Discord após cada partida confirmada.
// Autenticado por secret compartilhado (header X-Matchmaker-Secret), não por sessão de usuário.
async function handleMatchmakerSync(request, env, cors) {
    const secret = request.headers.get('X-Matchmaker-Secret');
    if (!secret || !env.MATCHMAKER_SECRET || secret !== env.MATCHMAKER_SECRET) {
        return json({ error: 'Unauthorized' }, 401, cors);
    }

    const body = await request.json();
    const { matchId, winner, loser } = body;
    if (!matchId || !winner?.discordId || !loser?.discordId) {
        return json({ error: 'payload inválido' }, 400, cors);
    }

    for (const p of [winner, loser]) {
        await mmPutPlayer(env, {
            discordId: p.discordId,
            username:  String(p.username || 'Desconhecido').slice(0, 64),
            elo:       Number(p.elo) || 1000,
            wins:      Number(p.wins) || 0,
            losses:    Number(p.losses) || 0,
            updatedAt: new Date().toISOString(),
        });
        await appendIndex(env, MM_PLAYER_INDEX_KEY, p.discordId);
    }

    await env.AUTH_KV.put(`mm_match:${matchId}`, JSON.stringify({
        matchId,
        winner: { discordId: winner.discordId, username: winner.username, delta: winner.delta },
        loser:  { discordId: loser.discordId,  username: loser.username,  delta: loser.delta },
        resolvedAt: new Date().toISOString(),
    }));
    await mmPrependIndex(env, MM_MATCH_INDEX_KEY, String(matchId), MM_MAX_MATCH_HISTORY);

    return json({ ok: true }, 200, cors);
}

// GET /matchmaker/ranking?limit=10 — leitura pública, sem sessão (mesmo padrão do /banner)
async function handleMatchmakerRanking(request, env, cors, url) {
    const limit = Number(url.searchParams.get('limit') || 10);
    const raw = await env.AUTH_KV.get(MM_PLAYER_INDEX_KEY);
    const ids = raw ? JSON.parse(raw) : [];
    const players = (await Promise.all(ids.map(id => mmGetPlayer(env, id)))).filter(Boolean);
    players.sort((a, b) => b.elo - a.elo);
    return json({ players: players.slice(0, limit) }, 200, cors);
}

// GET /matchmaker/player/:discordId — perfil público de um jogador
async function handleMatchmakerPlayer(request, env, cors, discordId) {
    const player = await mmGetPlayer(env, discordId);
    if (!player) return json({ error: 'Jogador não encontrado' }, 404, cors);
    return json({ player }, 200, cors);
}

// GET /matchmaker/matches?limit=20 — histórico recente de partidas confirmadas
async function handleMatchmakerMatches(request, env, cors, url) {
    const limit = Number(url.searchParams.get('limit') || 20);
    const raw = await env.AUTH_KV.get(MM_MATCH_INDEX_KEY);
    const ids = raw ? JSON.parse(raw).slice(0, limit) : [];
    const matches = (await Promise.all(
        ids.map(id => env.AUTH_KV.get(`mm_match:${id}`).then(v => v ? JSON.parse(v) : null))
    )).filter(Boolean);
...
export default {
    async fetch(request, env) {
        ...
        try {
            ...
            if (path === '/admin/promote-by-email' && method === 'POST') return handlePromoteByEmail(request, env, cors);
            ...
            if (path === '/cache-all' && method === 'GET') return handleCacheAllGet(request, env, cors);
            ...
            if (path === '/agent/chat' && method === 'POST') return handleAgentChat(request, env, cors);

            if (path === '/stores' && method === 'GET')  return handleStoresGet(request, env, cors);
            if (path === '/stores' && method === 'POST') return handleStoresPost(request, env, cors);

            const storeMatch = path.match(/^\/stores\/([^/]+)$/);
            if (storeMatch && method === 'PUT')    return handleStorePut(request, env, cors, storeMatch[1]);
            if (storeMatch && method === 'DELETE') return handleStoreDelete(request, env, cors, storeMatch[1]);

            if (path === '/deckmaps' && method === 'GET')  return handleDeckMapsGet(request, env, cors);
            if (path === '/deckmaps' && method === 'POST') return handleDeckMapsPost(request, env, cors);

            const deckMapMatch = path.match(/^\/deckmaps\/([^/]+)$/);
            if (deckMapMatch && method === 'PUT')    return handleDeckMapPut(request, env, cors, deckMapMatch[1]);
            if (deckMapMatch && method === 'DELETE') return handleDeckMapDelete(request, env, cors, deckMapMatch[1]);

            if (path === '/matchmaker/sync'    && method === 'POST') return handleMatchmakerSync(request, env, cors);
            if (path === '/matchmaker/ranking' && method === 'GET')  return handleMatchmakerRanking(request, env, cors, url);
            if (path === '/matchmaker/matches' && method === 'GET')  return handleMatchmakerMatches(request, env, cors, url);
